package com.jpa.yeongaesomun.config;
...
//    사용자가 입력한 비밀번호는 passwordEncoder()를 사용한다.
    ...
    //시큐리티가 관리하지 않을 파일들 static, 등등 경로 설정
    @Bean
...
//                .and()
                ...
                .authorizeRequests()
                .antMatchers(MAIN_PATH).authenticated() // 보안 검사를 하겠다.
                .antMatchers(ADMIN_PATH).hasRole(MemberRole.ADMIN.name()) //ADMIN 권한이 있는 회원은 접근 가능
                .antMatchers(BOARD_PATH).hasRole(MemberRole.USER.name())
                .and()
                .oauth2Login()
                .userInfoEndpoint();
